...
class SearchService:
    def __init__(self):
        ...
        print(f"⏱️ Векторизация заняла {time.time() - start:.2f} сек")

        print(f"✅ SearchService инициализирован. Товаров: {len(self.df)}")
        print(f"⏱️ Общее время инициализации: {time.time() - start_total:.2f} сек")

    def hybrid_search(self, query, top_k=10, exclude_skus=None, category=None, offset=0):
        """Поиск через TF-IDF с фильтрацией"""
        # Векторизуем запрос
        ...
        # Берём больше кандидатов для фильтрации
        candidate_indices = indices[:top_k * 3]

        # Фильтруем по категории (если указана)
        if category:
            filtered = []
            for idx in candidate_indices:
                if category.lower() in str(self.df.iloc[idx].get('category', '')).lower():
                    filtered.append(idx)
        ...
        # Если ничего не осталось — возвращаем пустой DataFrame
        if len(candidate_indices) == 0:
            return pd.DataFrame()

        # ========== РАНДОМИЗАЦИЯ С OFFSET ==========
        if len(candidate_indices) > top_k:
            # Берём блок с учётом смещения (циклически)
            start = offset % max(1, len(candidate_indices) - top_k)
            result_indices = candidate_indices[start:start + top_k]
            # Если не хватило — добираем с начала
            while len(result_indices) < top_k:
                result_indices.extend(candidate_indices[:top_k - len(result_indices)])
        else:
            result_indices = candidate_indices
        # ===========================================

        # Возвращаем топ-k результатов
        # result_indices = candidate_indices[:top_k]
        return self.df.iloc[result_indices]

    def search(self, query, top_k=5):
        """Упрощённый поиск для быстрых запросов"""
    ...
    def get_fallback_products(self, query: str, last_category: str = None, limit: int = 5):
        ...
        # Пробуем поиск по запросу
        results = self.hybrid_search(query, top_k=limit)
        if len(results) > 0:
            return self._to_items(results)

        # Если ничего не найдено — первые товары из каталога
